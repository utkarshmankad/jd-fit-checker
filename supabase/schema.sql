...
-- User-reviewed recommendation mistakes, used as private calibration memory.
...
create index if not exists recommendation_corrections_user_created_idx on public.recommendation_corrections(user_id, created_at desc);
alter table public.recommendation_corrections enable row level security;
create policy "recommendation_corrections_select_own" on public.recommendation_corrections for select to authenticated using ((select auth.uid()) = user_id);
create policy "recommendation_corrections_insert_own" on public.recommendation_corrections for insert to authenticated with check ((select auth.uid()) = user_id);
create policy "recommendation_corrections_update_own" on public.recommendation_corrections for update to authenticated using ((select auth.uid()) = user_id) with check ((select auth.uid()) = user_id);
create policy "recommendation_corrections_delete_own" on public.recommendation_corrections for delete to authenticated using ((select auth.uid()) = user_id);
...
-- ──────────────────────────────────────────────────────────
-- 14. Candidate evidence knowledge base (RAG Phase 1)
--    Private resume evidence chunks used by the fast hybrid retriever.
-- ──────────────────────────────────────────────────────────
...
create index if not exists candidate_evidence_user_id_idx on public.candidate_evidence(user_id);
create index if not exists candidate_evidence_search_idx on public.candidate_evidence using gin(search_vector);
create index if not exists candidate_evidence_skills_idx on public.candidate_evidence using gin(skills);
