...
function SkeletonRow({ showAllColumns }: { showAllColumns: boolean }) {
  return (
    <tr className="border-b border-gray-100">
      ...
      <td className="px-4 py-4 text-center">
        <div className="h-5 bg-gray-200 rounded-full animate-pulse w-24 mx-auto" />
      </td>
      <td className="px-4 py-4">
        <div className="h-4 bg-gray-200 rounded animate-pulse w-6 ml-auto" />
      </td>
    </tr>
  )
}
...
export default function DashboardPage() {
  ...
  const verdictCounts = {
    ...
    REJECT: goodResults.filter((r) => r.verdict === 'REJECT').length,
  }
      ...
      {!profileBannerDismissed && hasApiKey === true && !hasPreferences && (
        <div className="flex items-start gap-3 px-4 py-3 rounded-lg bg-amber-50 border border-amber-200 text-amber-800 text-sm">
          ...
          <span className="flex-1">
            ...
            <a href="/profile" className="font-semibold underline">Add your resume or set preferences → Set up profile</a>
          </span>
        ...
        <div className="px-6 pt-6 pb-0">
          ...
          <div className="flex border-b border-gray-200">
            {(['urls', 'text'] as InputTab[]).map((t) => (
              <button key={t} onClick={() => { setTab(t); setResults([]); setBatchTime(null); setScreenError(null) }}
              ...
              <div className="rounded-lg border border-gray-200 overflow-hidden">
                ...
                {showLinkedInHelper && (
                  <div className="border-t border-gray-100 px-4 py-4 space-y-4 bg-gray-50 text-sm text-gray-700">
                    <p className="text-xs text-gray-500">Run this script in your browser console while on the LinkedIn recommended jobs page. It auto-scrolls, collects up to 20 jobs, and copies the URLs.</p>
                    <ol className="space-y-3 text-sm">
                      ...
                      <li className="flex gap-2"><span className="font-bold text-gray-400 shrink-0">2.</span><span>Open DevTools — <kbd className="px-1.5 py-0.5 rounded bg-gray-200 text-gray-700 font-mono text-xs">F12</kbd> on Windows/Linux or <kbd className="px-1.5 py-0.5 rounded bg-gray-200 text-gray-700 font-mono text-xs">Cmd ⌥ J</kbd> on Mac — Console tab.</span></li>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {sortedResults.map((result) => {
                  ...
                  return (
                    <Fragment key={rowKey}>
                          ...
                          {isErrorRow ? (
                            ...
                            <div>
                              ...
                              {isReject && result.hard_reject_reasons?.length > 0 && (
                                <p className="text-xs text-red-600 font-normal mt-0.5 max-w-[200px] leading-snug">
                                  {result.hard_reject_reasons[0]}
                                </p>
                              )}
                            </div>
                          )}
                      ...
                      {/* Expanded row — always rendered, animated via max-height */}
                            ...
                            <div className="px-6 py-5">
                              {isErrorRow ? (
                                ...
                                <div className="space-y-4 max-w-3xl">
                                  ...
                                  <RecommendationBlock text={result.analysis_json?.recommendation ?? ''} />
                                </div>
                              )}
                            </div>
                          </div>
                ...
                {/* Skeleton rows during progressive load */}
