...
export async function checkScreenLimit(
  ...
  if (profile.is_beta_user || LAUNCH_MODE) {
    ...
    if (used + requested_count > BETA_LIMIT) {
      return {
        ...
        is_paid: false,
        reset_message: null,
        upgrade_prompt: `You've used all ${BETA_LIMIT} free judgments. Upgrade for ₹499 one-time — unlimited judgments, forever.`,
      }
    }
  ...
  if (used + requested_count > effective_limit) {
    ...
    return {
      ...
      is_beta: false,
      is_paid: false,
      reset_message: `Your ${WEEKLY_LIMIT} free judgments reset on ${reset_date.toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'short' })}.`,
      upgrade_prompt: bonus > 0
        ? `You've used your ${effective_limit} weekly judgments (${WEEKLY_LIMIT} base + ${bonus} referral bonus). Upgrade for ₹499 one-time — unlimited judgments, forever.`
        : `You've used your ${WEEKLY_LIMIT} free weekly judgments. Refer a friend for +10 bonus judgments, or upgrade for ₹499 one-time — unlimited judgments, forever.`,
    }
  }
